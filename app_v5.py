...
# --- RECURSO: JANELA DE BUSCA GLOBAL (MODIFICADO) ---
        ...
        for result_type, data in results:
            if result_type == 'company':
                ...
                try:
                    ...
                    company_name = company_folder_name
...
class FileManagerApp(Tk):
    def __init__(self):
        super().__init__()
        self.title("Sistema de Gerenciamento de Arquivos v6.1")
        ...
        self.global_search_var = StringVar() # <-- NOVO
    ...
    def show_login_frame(self):
        ...
        self.title("Sistema de Gerenciamento de Arquivos v6.1 - Login")
    ...
    def show_main_app_frame(self): # <-- MODIFICADO
        for widget in self.container.winfo_children(): widget.destroy()
        self.title(f"Sistema de Gerenciamento de Arquivos v6.1 - Logado como: {self.current_user} ({self.user_role})")
    ...
    def start_global_search(self, event=None): # <-- MODIFICADO
        ...
        def search_thread_task():
            ...
            for root, dirs, files in os.walk(ROOT_DIR):
                ...
                # Procura por empresas (pastas no nível raiz das COMPANY_DIRS)
                is_company_base_dir = os.path.basename(os.path.normpath(root)) in COMPANY_DIRS
                if is_company_base_dir:
                    for d in dirs:
                        if "_" in d and search_term_lower in d.lower():
                            company_results.add(d)
    ...
    def navigate_to_path(self, path): # <-- Navegação para arquivos
    ...
    def navigate_to_company(self, company_folder_name): # <-- NOVO: Navegação para empresas
        # Assume que a melhor aba para mostrar a empresa é a de "Desenhos"
        target_tab_key = "Desenhos"
        if target_tab_key in self.tabs:
            self.notebook.select([i for i, k in enumerate(self.tabs.keys()) if k == target_tab_key][0])
            tab_widget = self.tabs[target_tab_key]
            if isinstance(tab_widget, FileBrowserTab):
                # Encontra a empresa na árvore de empresas e a seleciona
                if tab_widget.company_tree.exists(company_folder_name):
                    ...
                    # A seleção irá disparar o evento `on_company_select` que já tem a lógica de navegação.
...
# --- As classes de abas (FileBrowserTab, etc.) estão abaixo. Devido ao limite de tamanho, o código completo está
# --- implícito, com as funções já mostradas acima sendo parte de suas respectivas classes.
...
class FileBrowserTab:
    ...
    def create_action_buttons(self, parent):
        ...
        if os.path.basename(self.base_path) == "Desenhos":
    ...
    def filter_files(self, *args):
        ...
        # Re-attach all items before filtering
