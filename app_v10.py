...
def carregar_configuracao():
    """Lê o arquivo config.json e retorna o caminho raiz."""
    try:
        with open("config.json", 'r', encoding='utf-8') as f:
            config = json.load(f)
            caminho = config.get("Z:")
            if not caminho or not os.path.isdir(caminho):
                messagebox.showerror("Erro de Configuração", f"O caminho '{caminho}' não é válido ou não está acessível. Verifique o arquivo config.json e sua conexão com a rede.")
                return None
            return caminho
    except FileNotFoundError:
        messagebox.showerror("Erro Crítico", "Arquivo 'config.json' não encontrado! Crie o arquivo ao lado do executável com o 'caminho_raiz' do servidor.")
        return None
    except Exception as e:
        messagebox.showerror("Erro ao Ler Configuração", f"Não foi possível ler o arquivo 'config.json':\n{e}")
        return None

# Carrega o diretório raiz a partir da configuração
ROOT_DIR = carregar_configuracao()

# Se o ROOT_DIR não for carregado, o programa não pode continuar.
if not ROOT_DIR:
    sys.exit("Falha ao carregar o diretório raiz. Encerrando.")

# O resto das variáveis agora usará o caminho do servidor
...
def setup_environment():
    ...
    if not os.path.exists(USERS_FILE):
        # ATUALIZAÇÃO: Definindo usuário padrão como 'admin_master'
        users = {"Draco182019": {"password": hash_password("Draco182019"), "role": "admin_master"}}
        write_users(users)
...
class FileManagerApp(Tk):
    def __init__(self):
        super().__init__()
        self.title("Sistema de Gerenciamento de Arquivos v6.5")
    ...
    def show_login_frame(self):
        ...
        self.title("Sistema de Gerenciamento de Arquivos v6.5 - Login")
    ...
    def attempt_login(self, username, password):
        ...
        if user_data and verify_password(user_data.get("password"), password):
            ...
            # ATUALIZAÇÃO: Admin ou admin_master têm todas as permissões
            if self.user_role in ['admin', 'admin_master']:
    ...
    def show_main_app_frame(self):
        for widget in self.container.winfo_children(): widget.destroy()
        self.title(f"Sistema de Gerenciamento de Arquivos v6.5 - Logado como: {self.current_user} ({self.user_role})")
    ...
    def create_main_menu(self):
        ...
        # ATUALIZAÇÃO: Apenas admin_master pode gerenciar usuários
    ...
    def open_user_management(self):
        # ATUALIZAÇÃO: Apenas admin_master pode acessar esta função
        if self.user_role != 'admin_master':
            messagebox.showerror("Acesso Negado", "Apenas o administrador master pode acessar esta função.")
            return
    ...
    def navigate_to_company(self, company_folder_name):
        target_tab_key = "Empresas" # Onde a árvore de empresas está
    ...
    def create_tabs(self):
        # ... (código da aba de Logs e perm_map continua igual)
        if self.user_role == 'admin_master':
            self.tabs["Logs"] = LogViewerTab(self.notebook, self)
            self.notebook.add(self.tabs["Logs"].parent_frame, text="📜 Logs do Sistema")
        ...
        for d in COMPANY_DIRS:
            if perm_map[d] != "none":
                # ATUALIZAÇÃO: Todas as abas de diretórios de empresa devem mostrar a árvore.
                self.tabs[d] = FileBrowserTab(self.notebook, self, os.path.join(ROOT_DIR, d), self.user_role, perm_map[d], show_company_tree=True)
    ...
    def refresh_all_tabs(self, refresh_companies=False, path_to_refresh=None):
        for tab in self.tabs.values():
            if isinstance(tab, FileBrowserTab):
                if refresh_companies and tab.show_company_tree: 
    ...
    def periodic_trash_cleanup(self):
        while True:
            time.sleep(3600)
            # ATUALIZAÇÃO: Apenas admin e admin_master podem limpar a lixeira automaticamente
            if self.user_role not in ['admin', 'admin_master']: continue
            ...
            if "Lixeira" in self.tabs and self.tabs["Lixeira"].parent_frame.winfo_exists():
                self.after(0, self.tabs["Lixeira"].populate_list)
...
class FileBrowserTab:
    def __init__(self, notebook, main_app, base_path, user_role, permission, show_company_tree=False):
        ...
        self.show_company_tree = show_company_tree
        ...
        # O painel esquerdo só é criado se necessário (aba Empresas)
        if self.show_company_tree:
    ...
    def create_action_buttons(self, parent):
        ...
        is_admin_or_master = self.user_role in ['admin', 'admin_master']
        can_edit = self.permission == 'edit'
        
        # ATUALIZAÇÃO: Condição corrigida para mostrar botões apenas na aba "Empresas".
        if os.path.basename(self.base_path) == "Empresas":
            self.create_button(frame, "Nova Empresa", self.create_new_company, "Cria estrutura de pastas para uma empresa.", 'normal' if is_admin_or_master else 'disabled')
            self.create_button(frame, "Excluir Empresa", self.delete_company, "Exclui PERMANENTEMENTE uma empresa.", 'normal' if is_admin_or_master else 'disabled')
    ...
    def paste_item(self):
        ...
        if not os.path.exists(source_path):
            messagebox.showerror("Erro", "O item de origem não existe mais."); self.main_app.clipboard_path = None; return
        ...
        if os.path.exists(dest_item_path):
            ...
            while os.path.exists(dest_item_path):
                dest_item_path = os.path.join(dest_path, f"{base}_copia({i}){ext}")
                i += 1
        ...
        try:
            if self.main_app.clipboard_operation == 'copy':
                ...
                self.main_app.refresh_all_tabs(path_to_refresh=source_dir)
    ...
    def populate_company_tree(self):
        ...
        try:
            # A base para listar empresas é sempre a pasta 'Empresas'
            company_base_dir = os.path.join(ROOT_DIR, "Empresas")
    ...
    def delete_company(self):
        if not self.show_company_tree: return
        ...
        try:
            ...
            self.main_app.refresh_all_tabs(refresh_companies=True, path_to_refresh=self.base_path)
        except Exception as e:
    ...
    def show_preview_frame(self):
        if not self.show_company_tree: return # Só mostra preview se houver painel esquerdo
...
class SpreadsheetManagerTab(BaseTab):
    def __init__(self, notebook, main_app, user_role, permission):
        ...
        action_frame = ttk.Frame(main_frame); action_frame.pack(fill='x', pady=5)
        can_edit = self.permission == 'edit'
        # ATUALIZAÇÃO: Botão "Nova Planilha" removido.
...
class UserManagementTab(BaseTab):
    def __init__(self, notebook, main_app):
        # ATUALIZAÇÃO: Apenas admin_master tem 'edit' aqui
        super().__init__(notebook, main_app, 'admin_master', 'edit')
        main_frame = ttk.Frame(self.parent_frame); main_frame.pack(expand=True, fill='both', padx=10, pady=10)
        action_frame = ttk.Frame(main_frame); action_frame.pack(fill='x', pady=5)
        
        # ATUALIZAÇÃO: Botões de adicionar/remover apenas para o admin_master
        is_master = self.main_app.user_role == 'admin_master'
        
        self.create_button(action_frame, "Adicionar Usuário", self.add_user, "Adiciona um novo usuário.", state='normal' if is_master else 'disabled')
        ...
        self.create_button(action_frame, "Remover Usuário", self.remove_user, "Remove o usuário.", state='normal' if is_master else 'disabled')
    ...
    def on_user_select(self, event):
        ...
        # O botão de editar só fica ativo se o usuário logado for master e o usuário selecionado não for ele mesmo
        can_edit = self.main_app.user_role == 'admin_master' and selected != self.main_app.current_user
        self.edit_perms_button.config(state='normal' if can_edit else 'disabled')
    ...
    def remove_user(self):
        ...
        users = read_users()
        if users.get(selected, {}).get("role") == "admin_master": return messagebox.showerror("Erro", "A conta de administrador master não pode ser removida.")
...
class LogViewerTab(BaseTab):
    def __init__(self, notebook, main_app):
        ...
        ttk.Label(search_frame, text="Buscar no Log:").pack(side='left', padx=(0, 5))
        self.search_var = StringVar()
        self.search_var.trace_add("write", self.filter_logs)
        search_entry = ttk.Entry(search_frame, textvariable=self.search_var)
        search_entry.pack(side='left', fill='x', expand=True)
        
        # Frame do texto do log
        text_frame = ttk.Frame(main_frame)
        text_frame.pack(expand=True, fill='both')
        
        self.log_text = Text(text_frame, wrap='word', font=("Courier New", 9), relief='sunken', borderwidth=1)
        ysb = ttk.Scrollbar(text_frame, orient='vertical', command=self.log_text.yview)
        self.log_text.config(yscrollcommand=ysb.set)
        
        ysb.pack(side='right', fill='y')
        self.log_text.pack(side='left', expand=True, fill='both')

        self.populate_log_display()

    def populate_log_display(self):
        self.log_text.config(state='normal')
        self.log_text.delete('1.0', 'end')
        try:
            with open(AUDIT_LOG_FILE, 'r', encoding='utf-8') as f:
                # Lê as linhas e as inverte para mostrar as mais recentes primeiro
                self.log_lines = f.readlines()[::-1]
                for line in self.log_lines:
                    self.log_text.insert('end', line)
        except FileNotFoundError:
            self.log_text.insert('1.0', "Arquivo de log de auditoria não encontrado.")
        except Exception as e:
            self.log_text.insert('1.0', f"Erro ao ler o arquivo de log:\n{e}")
    ...
    def filter_logs(self, *args):
        ...
        if not search_term:
            ...
            filtered_lines = [line for line in self.log_lines if search_term in line.lower()]
...
class PermissionsEditorWindow(Toplevel):
    def __init__(self, parent, user_mgmt_tab, username=None):
        ...
        # ATUALIZAÇÃO: Lógica para o Combobox de roles
        ...
        self.role_var.set('user') # Padrão é 'user'
        if not self.is_new_user:
            user_role = self.users_data.get(self.username, {}).get("role", "user")
            self.role_var.set(user_role)
            # Desabilita a troca de função se o usuário for admin ou master
            if user_role in ['admin', 'admin_master']:
    ...
    def toggle_permissions_frame(self, *args):
        state = 'disabled' if self.role_var.get() in ['admin', 'admin_master'] else 'normal'
    ...
    def save(self):
        if self.is_new_user:
            ...
            # ATUALIZAÇÃO: Altera a função apenas se não for admin/master
            current_role = self.users_data[self.username].get('role', 'user')
            if current_role not in ['admin', 'admin_master']:
                self.users_data[self.username]['role'] = self.role_var.get()

        if self.users_data[self.username]['role'] not in ['admin', 'admin_master']:
