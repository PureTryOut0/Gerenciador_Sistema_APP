...
# <<< NOVO: Lista de abas gerenciáveis
    ...
    if not os.path.exists(USERS_FILE):
        ...
        users = {
            admin_user: {
                ...
                "role": "admin"
                # <<< MUDANÇA: Admins não precisam de permissões explícitas
            }
        }
        write_users(users)
...
# ... (O resto das funções auxiliares como read_metadata, write_metadata, etc., continuam iguais) ...
def read_metadata():
...
class FileManagerApp(Tk):
    def __init__(self):
        super().__init__()
        self.title("Sistema de Gerenciamento de Arquivos v5.0")
    ...
    def show_login_frame(self):
        ...
        for widget in self.container.winfo_children():
            widget.destroy()
        self.title("Sistema de Gerenciamento de Arquivos v5.0 - Login")
    ...
    def attempt_login(self, username, password):
        ...
        if user_data and verify_password(user_data.get("password"), password):
            ...
            # <<< MUDANÇA: Carrega as permissões do usuário
            if self.user_role == 'admin':
                # Admin tem todas as permissões
                self.user_permissions = {tab: 'edit' for tab in MANAGEABLE_TABS}
            else:
                # Usuário comum tem permissões definidas no seu perfil
    ...
    def show_main_app_frame(self):
        """Mostra a interface principal da aplicação após o login."""
        for widget in self.container.winfo_children():
            widget.destroy()
        self.title(f"Sistema de Gerenciamento de Arquivos v5.0 - Logado como: {self.current_user} ({self.user_role})")
        
        self.notebook = ttk.Notebook(self.container)
        self.notebook.pack(expand=True, fill='both', padx=5, pady=5)

        self.tabs = {}
        self.create_tabs() # <<< MUDANÇA: A criação das abas agora depende das permissões
    ...
    def create_tabs(self):
        """Cria todas as abas da aplicação com base nas permissões."""
        # <<< MUDANÇA: Lógica de criação de abas totalmente refeita
        ...
        for dir_name in COMPANY_DIRS:
            ...
            if perm == "none":
                continue # Pula a criação da aba se não houver permissão
    ...
    def refresh_all_tabs(self):
        """Atualiza o conteúdo de todas as abas de navegação após uma restauração."""
    ...
    def periodic_trash_cleanup(self):
        """Verifica e limpa a lixeira periodicamente."""
        while True:
            time.sleep(3600)
            ...
            for item_name in items_to_remove:
                TrashManagerTab.delete_item_permanently(item_name)
...
# --- Classe da Tela de Login --- (Sem alterações)
...
class FileBrowserTab:
    # <<< MUDANÇA: Recebe a permissão específica para esta aba
    ...
    def create_action_buttons(self, parent):
        # <<< MUDANÇA: Botões agora dependem de `self.permission` ou `self.user_role`
    ...
    def show_context_menu(self, event):
        # <<< MUDANÇA: Menu de contexto também depende das permissões
    ...
    # O resto da classe FileBrowserTab (populate_file_tree, on_item_double_click, etc.) permanece o mesmo.
    # ... (código existente da classe FileBrowserTab sem alterações)...
    def load_icons(self):
        """Carrega ícones para a Treeview, criando-os em memória."""
        try:
            self.icon_folder = ImageTk.PhotoImage(Image.new('RGBA', (16, 16), (255, 193, 7, 255))) # Amarelo pasta
            self.icon_file = ImageTk.PhotoImage(Image.new('RGBA', (16, 16), (33, 150, 243, 255))) # Azul arquivo
            self.icon_pdf = ImageTk.PhotoImage(Image.new('RGBA', (16, 16), (244, 67, 54, 255)))   # Vermelho PDF
            self.icon_img = ImageTk.PhotoImage(Image.new('RGBA', (16, 16), (76, 175, 80, 255)))   # Verde Imagem
            self.icon_dxf = ImageTk.PhotoImage(Image.new('RGBA', (16, 16), (156, 39, 176, 255)))  # Roxo DXF
        except Exception:
            self.icon_folder = self.icon_file = self.icon_pdf = self.icon_img = self.icon_dxf = None

    def get_icon(self, filename):
        """Retorna o ícone apropriado com base na extensão."""
        ...
        try:
            for item in os.listdir(self.base_path):
                if os.path.isdir(os.path.join(self.base_path, item)):
                    ...
                    if len(parts) == 2:
                        self.company_tree.insert("", "end", values=parts, tags=('company',))
        except Exception as e:
    ...
    def delete_item(self):
        ...
        try:
            ...
            self.populate_file_tree(self.current_path)
            self.main_app.tabs["Lixeira"].populate_list()
        except Exception as e:
    ...
    def go_up(self):
        ...
        if os.path.normpath(parent_path) != os.path.normpath(self.current_path):
            self.populate_file_tree(parent_path)

    def on_company_select(self, event):
        selected_item = self.company_tree.focus()
        if not selected_item: return
        values = self.company_tree.item(selected_item, 'values')
        company_path = os.path.join(self.base_path, f"{values[0]}_{values[1]}")
    ...
    def on_item_double_click(self, event=None):
        ...
        else: open_file_with_default_app(item_path)
    ...
    def show_image_preview(self, path):
        ...
        except Exception as e: self.hide_preview()
    ...
    def show_dxf_preview(self, path):
        ...
        try:
            ...
            messagebox.showerror("Erro no Preview do DXF", f"Não foi possível renderizar o arquivo.\n\nDetalhe: {e}")
            self.hide_preview()


# --- Classe para Gerenciar a Lixeira ---
class TrashManagerTab:
    # <<< MUDANÇA: Recebe `permission`
    def __init__(self, parent_frame, main_app, user_role, permission):
        ...
        can_edit = self.permission == 'edit' # <<< MUDANÇA
        ...
        self.populate_list()
    # ... (O resto da classe TrashManagerTab continua igual) ...
    def create_button(self, parent, text, command, tooltip_text, state='normal'):
    ...
    def empty_trash(self):
        ...
        for item_name in list(read_metadata().get("trash", {}).keys()):
            self.delete_item_permanently(item_name)
        self.populate_list()
...
class SpreadsheetManagerTab:
    ...
    def __init__(self, parent_frame, main_app, user_role, permission):
        ...
        self.populate_list()
    # ... (O resto da classe SpreadsheetManagerTab continua igual) ...
    def create_button(self, parent, text, command, tooltip_text, state='normal'):
        button = ttk.Button(parent, text=text, command=command, state=state); button.pack(side='left', padx=5, pady=5)
        ToolTip(button, tooltip_text)
    ...
    def open_spreadsheet(self):
        selected_item = self.tree.focus()
        if not selected_item: return
        open_file_with_default_app(os.path.join(SPREADSHEETS_DIR, self.tree.item(selected_item, 'text').strip()))
...
# <<< NOVO: Classe para a janela de edição de permissões
class PermissionsEditorWindow(Toplevel):
    def __init__(self, parent, user_management_tab, username=None):
        ...
        # --- Frame de Permissões ---
        permissions_frame = ttk.LabelFrame(self, text="Permissões de Acesso às Abas", padding=10)
        permissions_frame.pack(padx=10, pady=10, fill="x")
        ...
        for i, tab_name in enumerate(MANAGEABLE_TABS):
            ttk.Label(permissions_frame, text=f"{tab_name}:").grid(row=i, column=0, sticky='w', padx=5, pady=5)
            
            perm_var = StringVar()
            combobox = ttk.Combobox(permissions_frame, textvariable=perm_var, values=list(permission_map.values()), state='readonly')
            
            # Define o valor padrão
            current_perm_key = current_permissions.get(tab_name, 'none') # Padrão é nenhum acesso
    ...
    def save_permissions(self):
        ...
        messagebox.showinfo("Sucesso", f"Permissões para '{self.username}' salvas com sucesso.")
...
class UserManagementTab:
    def __init__(self, parent_frame, main_app):
        ...
        self.tree.bind('<<TreeviewSelect>>', self.on_user_select) # <<< NOVO BIND
        
        self.populate_user_list()

    def on_user_select(self, event):
        # <<< NOVO: Habilita/desabilita o botão de editar
        selected_item = self.tree.focus()
        if not selected_item:
