...
class FileManagerApp(Tk):
    def __init__(self):
        super().__init__()
        self.title("Sistema de Gerenciamento de Arquivos v6.2")
    ...
    def show_login_frame(self):
        ...
        self.title("Sistema de Gerenciamento de Arquivos v6.2 - Login")
    ...
    def attempt_login(self, username, password):
        ...
        if user_data and verify_password(user_data.get("password"), password):
            ...
            # <<< ALERTA DE DEPURAÇÃO ADICIONADO AQUI >>>
    ...
    def show_main_app_frame(self):
        for widget in self.container.winfo_children(): widget.destroy()
        self.title(f"Sistema de Gerenciamento de Arquivos v6.2 - Logado como: {self.current_user} ({self.user_role})")
...
# --- As classes de abas (FileBrowserTab, etc.) estão abaixo, contendo todas as modificações.
...
class FileBrowserTab:
    ...
    def create_treeview(self, parent, columns):
        ...
        tree = ttk.Treeview(frame, columns=columns, show='tree headings')
        ysb = ttk.Scrollbar(frame, orient='vertical', command=tree.yview)
        tree.configure(yscrollcommand=ysb.set) # Barra de rolagem horizontal removida
        tree.pack(side='left', expand=True, fill='both')
        ysb.pack(side='right', fill='y')
        return tree
...
class SpreadsheetManagerTab(BaseTab):
    def __init__(self, notebook, main_app, user_role, permission):
        ...
        self.create_button(action_frame, "Carregar Planilha(s)", self.upload_spreadsheet, "Copia planilhas para a pasta do sistema.", 'normal' if can_edit else 'disabled') # <-- NOVO
    ...
    def upload_spreadsheet(self): # <-- NOVO
        ...
        for file_path in files:
            ...
            if os.path.exists(dest_path):
                if not messagebox.askyesno("Arquivo Existente", f"O arquivo '{os.path.basename(file_path)}' já existe.\nDeseja sobrescrevê-lo?"):
                    continue
            try:
                ...
                messagebox.showerror("Erro no Upload", f"Falha ao carregar '{os.path.basename(file_path)}':\n{e}")
        
        if uploaded_count > 0:
            messagebox.showinfo("Sucesso", f"{uploaded_count} planilha(s) carregada(s) com sucesso.")
            self.populate_list()
