...
class FileManagerApp(Tk):
    ...
    def show_main_app_frame(self):
        for widget in self.container.winfo_children():
            widget.destroy()
        self.title(f"Sistema de Gerenciamento de Arquivos v5.1 - Logado como: {self.current_user} ({self.user_role})")
...
class FileBrowserTab:
    ...
    # ... O resto das funções de FileBrowserTab que não mudaram ...
    def load_icons(self):
        try:
    ...
    # <<< INÍCIO DA INTEGRAÇÃO DO CÓDIGO FUNCIONAL >>>
    ...
    def show_dxf_preview(self, path):
        ...
        try:
            ...
            self.hide_preview()
    # <<< FIM DA INTEGRAÇÃO DO CÓDIGO FUNCIONAL >>>

# ... (O resto do código, como TrashManagerTab, SpreadsheetManagerTab, UserManagementTab, etc., continua igual ao da v5.0) ...
